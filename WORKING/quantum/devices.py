...
Device preference chain (probed once and cached):
    ...
    lightning.qubit  C++ multithreaded CPU simulator (OpenMP kernels)
    default.qubit    reference pure-Python simulator

The VQC layer resolves its simulator through this module. If an
accelerated backend is unavailable the pipeline falls back to the
reference backend unchanged, so the algorithm and training flow never
change; only the simulator kernel differs.

QAOA deliberately stays on default.qubit (see qaoa.py): accelerated
backends are slower for its small Hamiltonian-exp circuits and change
the selected-feature output.

GPU/CPU data placement: quantum states live inside the simulator and
never leave it; classical tensors stay on CPU because the classical
head is tiny and the GPU plugin is not installable on this Windows
host, so no unnecessary tensor/device transfers are introduced.
"""

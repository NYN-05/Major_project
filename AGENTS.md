...
- **No synthetic data.** The quantum layer consumes only the real rPPG feature table `output/rppg/dataset_features.csv` (3445 rows @native 30 fps, full extraction: 1883 real / 1562 fake — DFDC archive (`archive/DFDC_Dataset`, 1566 fake + 1727 real clips on disk) AND FF++ (train/val/test: FF-real + YouTube-real reals, FF-synthesis fakes); legacy `archive (1)` CSV layout is absent on disk). Rebuilt 2026-08-15 via `extract_dataset_features.py --include-ffpp --workers 0` (backup of prior 421-row table in `Scrape/dataset_features_backup_421rows_20260815.csv`). Never reintroduce a generator or a transform/bridge layer.
...
- **QAOA simulator (GPU-first):** `qaoa.simulator_device(wires, cfg)` returns `(dev, backend)` with backend in {torch, lightning, default}. `QAOASelectionConfig.device="auto"` (default) prefers a CUDA-capable torch backend (`default.qubit.torch`) gated by a one-time per-process probe (`qaoa._probe_gpu_device`): precision must reproduce the float64 classical cost within 1e-6 (the pipeline hard-asserts Hamiltonian ≡ classical cost) AND GPU per-call cost must be ≤ ~3× lightning.qubit. `"torch"` forces it (precision gate still applies; warns + falls back); `"lightning"`/`"default"` pin CPU. On this host PL 0.42 removed `default.qubit.torch` and `lightning.gpu` still has no Windows wheels (no `custatevec`, and building `cuquantum-python` requires a full CUDA Toolkit) → the probe fails with `backend=lightning` (verified: `Scrape/bench_gpu_backends.py`, per-call CPU ~0.02 ms vs GPU-transfer-overhead-dominated for 8 qubits). Cost circuits precompute `PauliRot` gates; restarts (default 4) run in a `ProcessPoolExecutor` (`QAOASelectionConfig.restarts`/`n_jobs`). Expect ~15 s vs ~37 s sequential on `default.qubit`.
...
- **VQC training device (GPU-first):** torch side of `HybridModel` (head, loss, optimizer) runs on CUDA when available (`vqc.resolve_device()`). The QNode simulator is GPU-first too: `VQCConfig.qnode_backend="auto"` (default) prefers the CUDA-capable `default.qubit.torch` when one exists and falls back to `default.qubit` + `backprop` on CPU (lightning.qubit adjoint was slower: 15.3 s vs 13.2 s). On PL 0.42 there is no GPU circuit backend, so the QNode is CPU today; measured CUDA-head gain: 15-epoch VQC train 12.44 s (CUDA head) vs 14.35 s (CPU-only head), min of 2 (`Scrape/bench_gpu_backends.py`). Inference: ~275 ms cold / 32 ms cached. On GPU hosts the 5-fold VQC CV in `evaluation.py` runs folds sequentially (`n_jobs=1`): parallel fold workers would each open a CUDA context on the shared 6 GB card (OOM risk); CPU hosts keep parallel folds.

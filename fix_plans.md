...
### H1 — Reject degenerate rPPG signals instead of fabricating neutral features
...
### H2 — Guard the rPPG cross-check so a bad pkl cannot crash the pipeline
...
### H3 — Rewrite the README data/model sections to describe the real system
...
### M1 — Fix broken frame-stage commands in the docs
...
### M2 — Sweep the "8 features" → 10-feature drift and complete the UI feature list
...
### M3 — Make the split configuration truthful
...
### M4 — Validate quantum artifact consistency in `predict_features`
...
### M5 — Fix `_fill_nan_with_median` docstring and the `-inf` SNR edge
...
### M6 — FrameIngestor: fall back to stride sampling when timestamps are unusable
...
### M7 — Worker crash visibility + parent timeout in extraction scripts
...
### M8 — Unbiased dataset caps and deterministic CSV row order (future extractions only)
...
### M9 — Data-quality transparency (documentation only)
...
### M10 — Make `--all` complete without xgboost
...
### M11 — Guard AUC computation for single-class folds
...
- **L1** `frontend/package.json:2` — `"name": "frontemd"` → `"frontend"`. *Why: correct metadata; no functional impact.*
- **L2** `WORKING/RPPG/rppg-pipeline/run_on_video.py:7` — docstring `examples/run_on_video.py` → `rppg-pipeline/run_on_video.py`.
- **L3** `streamlit_app.py:119-128` — model path via env `RPPG_CLASSIFIER_PKL` with the current default; leave the widget quirk. *Why: makes the demo usable from any cwd.*
- **L4** `WORKING/frame/app/pipeline.py:92-94` — remove the webcam `0 → 1` remap hack; pass the source through as-is. *Why: surprising undocumented behavior.*
- **L5** `WORKING/RPPG/rppg/model_utils.py` — record expected SHA-256 for `face_landmarker.task` and the Haar XML; verify after download, raise on mismatch; keep the `latest` URL but document it as a moving target. *Why: silent model drift breaks reproducibility.*
- **L6** ✅ already fixed this session (`AGENTS.md` + `RPPG/README.md` corrected) — no action.
- **L7** `WORKING/quantum/plots.py` — `_sklearn` docstring "~12 s" → "~2.4 s" (measured).
- **L8** Delete stale root `result.json` (untracked; references an FF++ clip; `/api/previous` reads `output/pipeline/` only). *Why: removes a misleading leftover artifact.*
- **L9** `git rm SKILL.md` — it is the frontend-design agent skill, already installed at `C:\Users\JHASHANK\.claude\skills\frontend-design\SKILL.md` (confirm the copy exists before deleting). *Why: unrelated clutter in a research repo.*
- **L10** `quantum/config.py:73` — add a comment: batch 32 with ≤32 train rows = single batch (benign). No code change.
- **L11** `quantum/vqc.py:58` — cache the `.cpu()` weight tensor across forwards (micro-opt). *Only if the inference path is touched anyway; otherwise skip.*
- **L12** `WORKING/frame/requirements.txt:9` — `opencv-python==4.6.0.66` → `>=4.8.0,<5.0.0` (matches RPPG + root merged file). *Why: removes the install-order foot-gun.*

---

## Phase 4 — Potential risks (documented; no code unless stated)

- **P1 — DFDC subject leakage (document only, per user decision).** `data.py:30-41` groups per clip because DFDC random 10-char IDs encode no subject pairing and the official DFDC subject-metadata JSON is unavailable. Add one sentence to the README data section: "DFDC clips are grouped per-clip (no subject IDs available); if the DFDC metadata JSON is obtained later, `_infer_subject_key` should map clips → subjects." No code change now.
- **P2 — Non-finite features at inference.** Add to `predict_features` (`quantum/pipeline.py`): `if not np.isfinite(x_scaled).all(): return INCONCLUSIVE payload` (guards the M5 `-inf` path end-to-end). Small, recommended.
- **P3 — GPU memory under 2 concurrent jobs.** Run a 2-job load test (two browser tabs). If OOM appears, add a frame-stage device knob (`--device cpu`) used under pressure. No change preemptively.
- **P4 — pkl trust.** Document in README/AGENTS: `output/rppg/` must remain write-protected; `rppg_classifier.pkl` is `pickle.load`-ed. No code change.
- **P5 — mediapipe pin.** Bump `mediapipe>=0.10.9` → `>=0.10.30` in `WORKING/RPPG/requirements.txt` (matches the Tasks API documented in `face_roi.py`); checksum pinning handled by L5.
- **P6 — split-balance edge (multi-clip groups).** Revisit only if P1 lands (subject metadata creates multi-clip groups).
...
| 7 | Doc integrity | `Select-String` for `469|669|8 feature|app/main.py` in root README | zero hits |
